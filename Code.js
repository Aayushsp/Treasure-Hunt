const accessCode1 = "VARIABLE";
const accessCode2 = "FUNCTION";
const accessCode3 = "DATABASE";

function clues() {
    
    fill("white")
    textSize(15)
    text("R E V B A I L A", 100,50)
    fill("lightblue")
    text("Hint: Always changing, not constant !!", 100,70)
// add code for displaying rest of the names and the hints.
    fill("white")
    textSize(15)
    text("C U T I N F O N", 640, 160)
    fill("lightblue")
    text("Hint: Block of code used to accomplish a specific task", 640, 180)

    fill("white")
    textSize(15)
    text("A T A D A S B E", 100, 300)
    fill("lightblue")
    text("Hint: A platform used to store, read and update data", 100, 320)
}